...
def stt_delivery(buy_value: float, sell_value: float) -> float:
    """STT on equity delivery: 0.1% both sides."""
    return (buy_value + sell_value) * 0.001


def stt_intraday(sell_value: float) -> float:
    """STT on equity intraday: 0.025% sell side only."""
    return sell_value * 0.00025


def stt_futures_sell(sell_value: float) -> float:
    """STT on futures: 0.02% sell side."""
    return sell_value * 0.0002


def stt_options_sell(sell_value: float) -> float:
    """STT on options: 0.1% sell side (on premium)."""
    return sell_value * 0.001
...
# Lot sizes (post Nov-2024 rationalization; NSE revises periodically)
...
# Weekly-expiry indices (post-2024 SEBI rationalization: one weekly expiry per exchange)
WEEKLY_EXPIRY_INDICES = {"NIFTY", "SENSEX"}  # NSE: NIFTY Tue; BSE: SENSEX Thu


def _third_wednesday(year: int, month: int) -> date:
    """Monthly F&O expiry convention: third Wednesday of the month."""
...
def upcoming_expiries(symbol: str, from_date: date, count: int = 4) -> list[date]:
    """Next `count` expiry dates for an index (skips trading holidays)."""
    symbol = symbol.upper()
    out: list[date] = []
    weekly_day = 1 if symbol in ("NIFTY",) else 3 if symbol == "SENSEX" else None  # Tue / Thu
    cursor = from_date
    year = cursor.year
    while len(out) < count:
        if weekly_day is not None and symbol in WEEKLY_EXPIRY_INDICES:
            candidate = _next_weekday(cursor, weekly_day)
            while is_trading_holiday(candidate):  # holiday → expiry moves to previous day
                candidate = date.fromordinal(candidate.toordinal() - 1)
            out.append(candidate)
            cursor = candidate + timedelta(days=1)
            year = candidate.year
            # Also inject monthly expiry when we cross its week? Keep simple: weekly only.
        else:
            # Monthly-only index (BANKNIFTY etc.) or fallback
            month = cursor.month
            candidate = _third_wednesday(year, month)
            if candidate < cursor:
                month += 1
                if month > 12:
                    month = 1
            ...
            nxt_month = candidate.month % 12 + 1
            nxt_year = candidate.year + (1 if candidate.month == 12 else 0)
            cursor = max(cursor, date(nxt_year, nxt_month, 1))
            year = nxt_year
